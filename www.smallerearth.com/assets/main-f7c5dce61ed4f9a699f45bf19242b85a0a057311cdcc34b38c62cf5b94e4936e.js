...
(function(t, e) {
  ...
  function t(t) {
    ...
    (o.elements = c(t.options)),
      ...
      o.destroyed &&
        ((o.destroyed = !1),
        t.options.container &&
          p(t.options.container, function(t) {
  ...
  function e(t) {
    for (var e = t._util, n = 0; n < e.count; n++) {
      ...
      s.container && g && (a = a.closest(s.containerClass))
        ...
        : (s = o(c, m)),
        (s || r(i, t.options.successClass)) &&
          (t.load(i), e.elements.splice(n, 1), e.count--, n--);
    }
    0 === e.count && t.destroy();
  }
  function o(t, e) {
  ...
  function n(t, e, o) {
    if (
      !r(t, o.successClass) &&
      (e || o.loadInvisible || (0 < t.offsetWidth && 0 < t.offsetHeight))
    )
      if ((e = t.getAttribute(v) || t.getAttribute(o.src))) {
        ...
        'video' === t.nodeName.toLowerCase()
          ? (p(t.getElementsByTagName('source'), function(t) {
              var e = o.src,
                n = t.getAttribute(e);
    ...
    if (!document.querySelectorAll) {
      ...
      document.querySelectorAll = function(t, e, o, n, i) {
        for (
          ...
          for (s.addRule(t[o], 'k:v'), n = i.length; n--; )
    ...
    (r.elements = []),
      ...
      (i.destroy = function() {
        ...
        this.options.container &&
          p(this.options.container, function(e) {
      ...
      p(i.options.breakpoints, function(t) {
        if (t.width >= window.screen.width) return (v = t.src), !1;
      }),
      setTimeout(function() {
        t(i);
      });
...
var formLabels = document.querySelectorAll('.apply__label');
if (formLabels) {
  if (navigator.appName == 'Microsoft Internet Explorer' || !!(navigator.userAgent.match(/Trident/) || navigator.userAgent.match(/rv:11/)) || typeof $.browser !== 'undefined' && $.browser.msie == 1) {
    var i = undefined;
    for (i = 0; i < formLabels.length; i++) {
      formLabels[i].classList.add('active');
    }
  }
}
;
...
if (accordionHeader) {
  ...
  for (i = 0; i < accordionHeader.length; i++) {
    accordionHeader[i].addEventListener('click', function (e) {
      var accordionParent = e.target.parentElement.classList.contains('active');
      if (accordionParent) {
...
var collapseHeader = document.querySelectorAll('.block__collapse .block__title--wrap');

if (collapseHeader) {
  var i = undefined;
  for (i = 0; i < collapseHeader.length; i++) {
    collapseHeader[i].addEventListener('click', function (e) {
      var collapseParent = e.target.parentElement.classList.contains('active');
      if (collapseParent) {
        e.target.parentElement.classList.remove('active');
      } else {
        e.target.parentElement.classList.add('active');
...
var countryNav = document.querySelector('.navigation.navigation__country--change');
var countryNavClose = document.querySelector('#country__change--close');
var countryBtnOpen = document.querySelectorAll('.country__change--btn_open');
var countryBtnClose = document.querySelector('.country__change--btn_close');

if (countryBtnOpen) {
  countryBtnOpen.forEach(function (el) {
    el.addEventListener('click', function () {
      document.querySelector('.overlay').classList.add('active');
      document.querySelectorAll('.navigation')[0].classList.remove('active');
      countryNav.classList.add('active');
    });
  });
}

if (countryBtnClose) {
  countryBtnClose.addEventListener('click', function () {
    countryNav.classList.remove('active');
    document.querySelectorAll('.navigation')[0].classList.add('active');
  });
}

if (countryNavClose) {
  countryNavClose.addEventListener('click', function () {
    countryNav.classList.remove('active');
    unFreeze();
  });
}
...
var customSelectBtn = document.querySelectorAll('.custom__select--btn');

if (customSelectBtn) {
  var i = undefined;
  for (i = 0; i < customSelectBtn.length; i++) {
    customSelectBtn[i].addEventListener('click', function (e) {
      var customSelectBtnParent = e.target.parentElement.classList.contains('active');
      if (customSelectBtnParent) {
        e.target.parentElement.classList.remove('active');
      } else {
        e.target.parentElement.classList.add('active');
      }
...
for (i = 0; i < navItems.length; i++) {
  navItems[i].addEventListener('click', function (e) {
    var result = e.target.parentElement.classList.contains('active');
...
var regions = document.querySelectorAll('.events__slider--region');
var regionSelect = document.querySelector('.events__slider--select');

if (regions) {
  var _i = undefined;
  for (_i = 0; _i < regions.length; _i++) {
    if (_i !== 0) {
      regions[_i].classList.add('event__hide');
    }
  }
}
...
if (regionSelect) {
  ...
  var regionTitles = document.querySelectorAll('.events__slider--region__title');

  for (i = 0; i < regionTitles.length; i++) {
    var option = document.createElement('option');
    option.text = regionTitles[i].textContent;
    option.id = regionTitles[i].textContent.toLowerCase().replace(/ /g, '');
    option.value = i;
  ...
  if (urlHash.length > 0) {
    var opt = [];
    var _i2 = undefined;
    for (_i2 = 0; _i2 < regionSelect.options.length; _i2++) {
      opt.push(regionSelect.options[_i2].id);
    }

    regionSelect.value = opt.indexOf(urlHash);

    for (_i2 = 0; _i2 < regions.length; _i2++) {
      if (_i2 !== 0) {
        regions[_i2].classList.add('event__hide');
      }
    }

    window.location.hash = regionSelect.selectedOptions[0].id;
    regions[opt.indexOf(urlHash)].classList.remove('event__hide');
  }

  regionSelect.addEventListener('change', function (e) {
    for (i = 0; i < regions.length; i++) {
      if (!regions[i].classList.contains('event__hide')) {
        regions[i].classList.add('event__hide');
      }
    }

    regions[e.target.value].classList.remove('event__hide');
    window.location.hash = regionSelect.selectedOptions[0].id;
  });
}
;
...
var faqSel = document.querySelector('.faq__answers--select');
var faqSelInput = document.querySelector('#faq__answers--select_input');
var faqAnswerSidebar = document.querySelector('.faq__answers--sidebar');

if (faqSelInput) {
  faqSelInput.addEventListener('change', function (e) {
    window.location = e.target.value;
  });
}

if (window.outerWidth < 768 && faqAnswerSidebar) {
  window.addEventListener('scroll', function (e) {
    if (faqAnswerSidebar.getBoundingClientRect().top <= 0) {
      faqSel.classList.add('active');
    } else {
      faqSel.classList.remove('active');
    }
  });
}
;
...
var subNavItems = document.querySelectorAll('.sub__navigation--item');
var subNavWrap = document.querySelector('.sub__navigation--wrap');

var subNavItemsWidth = 0;
...
if (subNavItems && subNavWrap) {
  (function () {
    ...
    for (i = 0; i < subNavItems.length; i++) {
      subNavItemsWidth += subNavItems[i].offsetWidth;
    }
...
if (tables) {
  ...
  for (i = 0; i < tables.length; i++) {
    if (tables[i].offsetWidth > window.outerWidth) {
...
var teamMember = document.querySelectorAll('.team__member');
  ...
  (function () {
    ...
    for (i = 0; i < teamMember.length; i++) {
      teamMember[i].addEventListener('click', function (e) {
        var activeMember = e.target.classList.contains('active');
        if (activeMember) {
...
var closeSwitch = document.querySelector('.suggest_country_close');
var suggestBox = document.querySelector('.header__suggest_switch');

if (closeSwitch) {
  var handleCloseSwitch = function handleCloseSwitch() {
    suggestBox.classList.add('hide');
    localStorage.setItem('dismissed_suggest_country', "true");
  };

  closeSwitch.addEventListener('click', handleCloseSwitch);

  if (localStorage.getItem('dismissed_suggest_country') != "true") {
    suggestBox.classList.remove('hide');
  }
}
;
$(document).ready(function() {
  var $daySlider = $('#day_slider');

  if ($daySlider.length) {
    var updateRangeSlider = function() {
      var $getSlides = $('.range__slider--slides .slick-slide');
      $daySlider.attr('max', $getSlides.length - 1);
    };

    var goToSlider = function() {
  ...
  var $rangeSlider = $('.range__slider--slides');

  if ($rangeSlider.length && $daySlider.length) {
    $rangeSlider.slick({
      infinite: false,
      slidesToShow: 1,
      slidesToScroll: 1,
      prevArrow: $('.range-prev'),
      nextArrow: $('.range-next'),
    ...
    $daySlider.on('input', function(e) {
      $('.range__slider--slides').slick('slickGoTo', e.target.value, 0);
    });

    $rangeSlider.on('beforeChange', function(
      event,
      slick,
      currentSlide,
      nextSlide
...
$(document).ready(function() {
  ...
  $blogSlider.each(function() {
    ...
    slickInduvidual.slick({
      ...
      slidesToScroll: 1,
      arrows: true,
      prevArrow: $('.blog__slide--prev'),
      nextArrow: $('.blog__slide--next'),
      waitForAnimate: 1,
      dots: false,
      responsive: [
        {
    ...
    slickInduvidual.on('click', '.slick-slide', function(e) {
      e.stopPropagation();
      var index = $(this).data('slick-index');
      if (slickInduvidual.slick('slickCurrentSlide') !== index) {
        slickInduvidual.slick('slickGoTo', index);
      }
    });
  });
});
$(document).ready(function() {
  // EVENTS
  var $eventsSlider = $('.events__slider--region__slides');

  $eventsSlider.each(function() {
    var eventsInduvidual = $(this);

    eventsInduvidual.slick({
      infinite: true,
      slidesToShow: 1,
      slidesToScroll: 1,
      arrows: false,
...
$(document).ready(function() {
  ...
  $heroSlider.on('beforeChange', function(slick, currentSlide, nextSlide) {
    console.log(slick);
  });
...
$(document).ready(function() {
  $('.image__gallery_image').magnificPopup({
    type: 'image',
    gallery: {
      enabled: true
    }
